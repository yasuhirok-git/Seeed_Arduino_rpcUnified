...
// rpc_wifi_lwip interface rpc_lwip_select2 function client shim.
int32_t rpc_lwip_select2(int32_t maxfdp1, const binary_t * readset_in, const binary_t * writeset_in, const binary_t * exceptset_in, const binary_t * timeout, binary_t * fdset_out)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    {
        codec->startWriteMessage(kInvocationMessage, krpc_wifi_lwip_service_id, krpc_wifi_lwip_rpc_lwip_select2_id, request.getSequence());
        ...
        {
            codec->writeNullFlag(false);
            write_binary_t_struct(codec, readset_in);
        }
        ...
        {
            codec->writeNullFlag(false);
            write_binary_t_struct(codec, writeset_in);
        }

        if (exceptset_in == NULL)
        {
            codec->writeNullFlag(true);
        }
        else
        {
            codec->writeNullFlag(false);
            write_binary_t_struct(codec, exceptset_in);
        }
        ...
        read_binary_t_struct(codec, fdset_out);
    ...
    // Invoke error handler callback function
    g_client->callErrorHandler(err, krpc_wifi_lwip_rpc_lwip_select2_id);
